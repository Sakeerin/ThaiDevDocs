<template>
  <AuthPageHeader title="เข้าสู่ระบบ" @submit="login">
    <template #subtitle>
      ยังไม่มีบัญชี?
...
const login = async () => {
  ...
  try {
    await authLogin(form.email, form.password, form.remember)
    const redirect = (route.query.redirect as string) || '/'
    router.push(redirect)
  } catch (e: any) {
    error.value = e?.error?.message || e?.message || 'อีเมลหรือรหัสผ่านไม่ถูกต้อง'
  } finally {
